...
export default function UserPage() {
  ...
  async function sendWhatsApp(contact: Contact) {
    ...
    try {
      ...
      // Optimistic Update: instantly mark as sent in local state
      setContacts((prev) =>
        prev.map((c) =>
          c.id === contact.id
            ? { ...c, waSent: true, waSentAt: new Date(), waSentBy: user.uid }
            : c
        )
      );

      // 4. Mark as sent in Firestore database
      await updateContactWaSent(contact.id, user.uid);
      showToast("✓ Message marked as sent.");
    } catch (err) {
      // Rollback optimistic state update on error
      setContacts(originalContacts);
      showToast(err instanceof Error ? `❌ Error: ${err.message}` : "❌ Failed to update status.");
    } finally {
      setSendingId(null);
    }
  }
  ...
  return (
    <ProtectedRoute>
            ...
            {visible.length === 0 ? (
              ...
              visible.map((contact) => (
                ...
                >
                  ...
                  <div className="mt-4">
                    {contact.waSent ? (
                      <div className="flex h-11 items-center justify-center gap-2 rounded-xl bg-emerald-500/10 text-xs font-bold text-emerald-400 border border-emerald-500/20">
                        <svg
                        ...
                        >
                          ...
                          <path strokeLinecap="round" strokeLinejoin="round" d="M9 13l4 4L23 7" className="opacity-70" />
                        </svg>
                        ✓✓ Sent
                      </div>
                    ) : (
      ...
      {toast && (
        <div className="fixed bottom-5 right-5 z-50 rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/20 animate-fade-in">
          {toast}
        </div>
      )}
    </ProtectedRoute>
